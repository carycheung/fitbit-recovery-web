...
- **登录态也记住了**：授权拿到的 access token 存在浏览器（约 1 小时有效），有效期内**刷新页面纯本地 `fetch` 拉新、完全不碰 Google、不会卡**（这点对手机浏览器很关键）。
- **手机用法**：打开就显示缓存；要更新到今天，点右上角 **↻**。token 有效时一点即刷；过期了会弹一次 Google 授权窗口（手机上必须由这一下点击触发，才不会被拦/卡住）。
- 页面**不会**在打开时自动弹授权窗口——那正是之前手机上「拉取就停了」的原因（浏览器拦截了后台静默刷新）。所有联网都带超时看门狗，最多等 20–25 秒就给结果或报错，绝不无限转圈。
- 想强制刷新：右上角 **↻**，或 `⚙ → 重新拉取`。
