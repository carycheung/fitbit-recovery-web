...
- 拉过的数据缓存在浏览器 `localStorage`，**打开先秒显缓存里的「今天」**，离线也能看，**缓存里有就不强制联网更新**。
- 顶部 **↻ 同步** 按钮：手动点一下 → 自动拉最新数据 → dashboard 顶部显示「**最近更新 · 时间 · 几分钟前**」。同步时有一条**真实进度条**（按 7 类数据逐个完成推进，不是假动画），按钮转圈，结束停。
- **登录态**：access token 存浏览器。**注意 Google 硬性规定浏览器 token 最多 1 小时**——纯前端页面拿不到多天有效的 refresh token（那需要后端+密钥，与「无后端/无密钥」冲突）。但这不影响使用：
  - **凭据（Client ID + 已授权同意）是永久的**，你不用重填、不用重新同意。
  - token 过期后，**桌面端**打开会自动后台静默续期；**手机端**点一下 ↻ 即刷（手机浏览器拦截后台静默刷新，所以必须一次点击触发——这也正是之前「拉取就停了」的根因）。
  - 换句话说：**看数据永远不需要登录（缓存 7 天+可用），只有要拉「今天的新数据」时，手机上点一下 ↻。**
- 页面**不会**在打开时自动弹授权窗口。所有联网都带超时看门狗（授权 25s / 每个请求 20s），最多等一会就出结果或报错，**绝不无限转圈**。
